...
const exampleService = require(path.resolve('./modules/example/services/example.service.js'))
...
module.exports = {
  ...
  createExample: async (ctx) => {
    ...
    if (types.includes(mimeType[1])) {
      ...
      const stream = fs.createWriteStream(path.resolve(`./static/assets/images/${filename}`))
  ...
  deleteExample: async (ctx) => {
    ...
    if (resultA.rowCount) {
      fs.unlinkSync(path.resolve(`./static/assets/images/${resultA.rows[0].image}`))
    }
  ...
  editExample: async (ctx) => {
    ...
    if (files) {
      ...
      if (types.includes(mimeType[1])) {
        ...
        resultB = await exampleService.editExample(exampleId, fields.name, filename)
      }
    } else {
