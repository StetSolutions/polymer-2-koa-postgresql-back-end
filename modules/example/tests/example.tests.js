...
test.serial('Get example by name', async t => {
  const res = await request
    .get(`/api/v1/examples/name/${example1.fields.name}`)
...
test.serial('Get examples paginated', async t => {
  const res = await request
    .get('/api/v1/examples/?offset=0&limit=1&column=name&direction=ASC')
...
test.serial('Delete example with non-existing id', async t => {
  const res = await request
    .del(`/api/v1/examples/1234567890`)
...
test.serial('Get by example name should not allow unauthenticated access', async t => {
  const res = await request
    .get(`/api/v1/examples/name/${example1.fields.name}`)
