...
test.serial('Get paginated', async t => {
  const res = await request
    .get('/api/v1/roles/?offset=0&limit=5&column=role_name&direction=ASC')
...
test.serial('Update', async t => {
  let res = await request
    .get('/api/v1/roles/?offset=0&limit=5&column=id&direction=DESC')

  const role = res.body.results[0]

  res = await request
    .post(`/api/v1/roles/${role.id}`)
    .send({
      role_name: 'super'
    })
