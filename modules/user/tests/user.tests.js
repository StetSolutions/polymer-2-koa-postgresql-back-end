...
test.serial('Get by id', async t => {
  let res = await request
    .get('/api/v1/users?limit=5&offset=0&column=created&direction=desc&search={}')
...
test.serial('Search term only', async t => {
  ...
  const res = await request
    .get(`/api/v1/users?limit=5&offset=0&column=created&direction=desc&search=${search}`)
...
test.serial('Update', async t => {
  ...
  res = await request
    .post(`/api/v1/users/profile/${userId}`)
    .send({
      email: dummy.email,
...
test.serial('Update password', async t => {
  const res = await request
    .post(`/api/v1/users/password/${user.id}`)
    .send({
      password: dummy.password
    })
...
test.serial('Check user by email', async t => {
  const res = await request
    .get(`/api/v1/users/email/${admin.email}`)
...
test.serial('Check user by username', async t => {
  const res = await request
    .get(`/api/v1/users/username/${admin.username}`)
...
test.serial('Update should not allow unauthorized access', async t => {
  const res = await request
    .post(`/api/v1/users/profile/${admin.id}`)
    .send({
      email: dummy.email,
      firstName: dummy.firstName,
      lastName: dummy.lastName,
...
test.serial('Update password should not allow unauthorized access', async t => {
  const res = await request
    .post(`/api/v1/users/password/${admin.id}`)
    .send({
      password: dummy.password
    })
...
test.serial('Delete', async t => {
  let res = await request
    .get('/api/v1/users?limit=5&offset=0&column=id&direction=desc&search={}')
